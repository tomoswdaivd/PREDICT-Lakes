...
## Full anomaly-persistence baseline

The second baseline defines the current state on the same 30-day scale as the targets. Its recent window is the 30 UTC calendar dates ending on and including the last complete daily state date. It requires at least 27 valid daily means. The current anomaly is the recent-window mean minus the direct matched-window climatology from at least three valid years strictly before the issue year. Full anomaly persistence then adds this single anomaly unchanged to each target-window climatology; its coefficient is fixed at 1.0 and is not fitted.

The paired weekly hindcast scores climatology and full anomaly persistence only where the recent anomaly, both forecasts, and the observed target are valid. For month 1 (N=397), full persistence changes bias from -0.417 to -0.034 degC, MAE from 0.964 to 0.839 degC, and RMSE from 1.237 to 1.132 degC. For month 2 (N=392), bias changes from -0.429 to -0.023 degC, but MAE rises from 0.982 to 1.161 degC and RMSE from 1.250 to 1.544 degC. For month 3 (N=388), bias changes from -0.404 to +0.032 degC, while MAE rises from 0.965 to 1.249 degC and RMSE from 1.240 to 1.710 degC. Full persistence therefore removes most of the mean cool bias, slightly overcorrecting it at month 3, but improves overall accuracy only for month 1.

The exploratory relationship between current and future anomalies weakens sharply with lead. Correlation and regression slope are 0.527 and 0.528 for month 1, 0.135 and 0.135 for month 2, and -0.066 and -0.066 for month 3. Deterministic issue-year block-bootstrap 95% intervals for the slope are [0.326, 0.627], [-0.057, 0.372], and [-0.334, 0.303], respectively. Only month 1 gives clear evidence of persistence under this uncertainty treatment. These regressions diagnose persistence and are not used to fit the forecasts.

Performance is season-dependent. Month-1 RMSE improves in DJF, MAM, and SON but worsens by 0.096 degC in JJA. Month-2 RMSE improves modestly in DJF and SON but worsens by 0.255 degC in MAM and 0.805 degC in JJA. Month-3 RMSE improves only in SON and worsens by 0.139, 0.642, and 0.995 degC in DJF, MAM, and JJA. A fixed full anomaly is therefore especially unsuitable at longer leads through spring and summer transitions.

Across 408 valid weekly current anomalies, the mean is +0.399 degC, standard deviation 1.159 degC, and 10th-to-90th percentile range -1.068 to +1.646 degC. Annual means vary non-monotonically from -0.190 degC in 2013 to +1.129 degC in 2014. A descriptive linear year slope is +0.043 degC per year with a normal-approximation 95% interval of [-0.006, +0.093] and correlation 0.084. This does not establish trend attribution or show that long-term warming caused the climatology bias; overlapping weekly windows and the short archive further limit that interpretation.

## Lead-dependent anomaly relaxation

An explicit audit confirmed that the preceding anomaly-survival slopes are ordinary unstandardised intercept-OLS coefficients: sample covariance divided by current-anomaly variance. For months 1-3, current-anomaly standard deviations are 1.165, 1.175, and 1.171 degC; future-anomaly standard deviations are 1.166, 1.176, and 1.174 degC. The respective covariance/variance calculations are 0.7163/1.3572=0.5278, 0.1863/1.3795=0.1350, and -0.0901/1.3717=-0.0657, with intercepts 0.215, 0.374, and 0.433 degC. Slopes resemble correlations because the two standard deviations are nearly equal, not because standardised coefficients were reported.

The relaxation model fits a separate non-seasonal beta for each horizon using zero-intercept OLS, `sum(current_anomaly * future_target_anomaly) / sum(current_anomaly^2)`, then constrains beta to [0,1]. For every issue year Y, only completed weekly cases issued in years below Y are eligible; their target windows must end before 1 January Y, so coefficients are fixed through Y without leaking early-year target outcomes. At least three distinct prior issue years are required, independently of the larger overlapping weekly-row count.

Coefficients first become valid in 2014. Month-1 constrained beta evolves from 0.455 in 2014 to 0.542, 0.567, 0.559, and 0.571 through 2018, showing reasonable stability near 0.55. Month 2 evolves from 0.064 to 0.236, 0.262, 0.283, and 0.299 rather than shrinking monotonically to zero. Month 3 has raw beta -0.138 (constrained to zero) in 2014, then 0.070, 0.075, 0.167, and 0.178. The positive later month-2/3 zero-intercept coefficients should not automatically be interpreted as physical memory: non-zero mean anomalies and climatology bias can influence a fit whose intercept is fixed to zero. For comparison, the full-period zero-intercept diagnostic slopes are 0.583, 0.234, and 0.055.

On identical cases for all three models, fitted relaxation improves month-1 MAE/RMSE from climatology's 0.935/1.196 to 0.699/0.920 degC (N=256), and improves slightly over full persistence's 0.707/0.986. Month 2 improves climatology only slightly, from 0.941/1.205 to 0.913/1.178 degC (N=251), while outperforming full persistence's 1.010/1.378. Month 3 changes climatology's 0.937/1.206 to 0.958/1.231 degC (N=247), so it is worse than climatology although much better than full persistence's 1.123/1.542. Relaxation reduces but does not remove the paired climatology cool bias: bias changes from -0.582 to -0.278, -0.578 to -0.461, and -0.570 to -0.522 degC for months 1-3.

Month-1 RMSE improves over climatology in every issue season (0.159-0.415 degC). Month-2 changes are modest: improvements in DJF, JJA, and SON and a 0.037 degC degradation in MAM. Month 3 improves slightly in DJF and SON but degrades by 0.053 degC in MAM and 0.075 degC in JJA. These results support a robust non-zero month-1 correction, only marginal practical value at month 2, and no current justification for a non-zero month-3 correction. Seasonal beta fitting has not been attempted.
